...
namespace CustomFeature\ClassRankerApi\Exceptions;
...
class Handler extends BaseHandler
{
        ...
        $this->renderable(function (AuthenticationException $exception, Request $request) {
            ...
            if ($request->wantsJson()) {
                return response()->json([
                    'error' => trans("class_ranker::app.{$nameSpace}.errors.401.message"),
                ], 401);
            }
    ...
    protected function handleHttpException(): void
    {
        $this->renderable(function (HttpException $exception, Request $request) {
            ...
            if ($request->wantsJson()) {
                return response()->json([
                    'title'   => trans("class_ranker::app.{$nameSpace}.errors.{$errorCode}.title"),
                    'message' => trans("class_ranker::app.{$nameSpace}.errors.{$errorCode}.message"),
                ], $errorCode);
            }
    ...
    protected function handleServerException(): void
    {
        $this->renderable(function (Throwable $throwable, Request $request) {
            ...
            if ($request->wantsJson()) {
                return response()->json([
                    'error' => trans("class_ranker::app.{$nameSpace}.errors.message"),
                ], $errorCode);
            }
