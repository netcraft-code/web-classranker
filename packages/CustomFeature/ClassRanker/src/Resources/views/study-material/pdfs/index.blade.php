<x-admin::layouts>
    <x-slot:title>
        @lang('class_ranker::app.study_materials.pdfs.index.title')
    </x-slot>
        ...
        <div class="flex items-center justify-between gap-4 max-sm:flex-wrap">
            ...
            <div class="flex items-center gap-x-2.5">
                <button type="button" class="primary-button">
                    @lang('class_ranker::app.study_materials.pdfs.index.create-btn')
                </button>
            </div>
        </div>
    ...
    @pushOnce('scripts')
        <script type="text/x-template" id="v-pdfs-template">
            ...
            <div class="flex items-center justify-between gap-4 max-sm:flex-wrap">
                ...
                <div class="flex items-center gap-x-2.5">
                    <button type="button" class="primary-button"
                        @click="resetForm(); $refs.pdfModal.toggle()"
                    >
                        @lang('class_ranker::app.study_materials.pdfs.index.create-btn')
                    </button>
                </div>
            ...
            <x-admin::datagrid :src="route('admin.study_materials.pdfs.index')" :isMultiRow="true">
                <template #header="{
                    isLoading,
                    available,
                    applied,
                    ...
                    <template v-else>
                        ...
                        >
                            <div
                                ...
                                v-for="(columnGroup, index) in [['id', 'created_at', 'status', 'is_premium'], ['slug', 'short_title', 'title'], ['board_name', 'grade_name', 'subject_name', 'book_title', 'chapter_title'], ['item_count']]"
                            >
            ...
            <x-admin::form v-slot="{ meta, errors, handleSubmit }" as="div" ref="modalForm">
                <form @submit="handleSubmit($event, createPdf)" ref="pdfForm">
                    <x-admin::modal ref="pdfModal" width="max-w-4xl">
                        <x-slot:header>
                            <p class="text-lg font-bold text-gray-800 dark:text-white">Create PDF</p>
                        </x-slot>
        ...
        <script type="module">
            app.component('v-pdfs', {
                template: '#v-pdfs-template',
                ...
                methods: {
                    ...
                    createPdf(params, { setErrors }) {
                        if (!this.assignments.length) {
                            alert('Please add at least one assignment'); return;
                        }
                        this.isLoading = true;
                        const formData = new FormData(this.$refs.pdfForm);

                        this.$axios.post("{{ route('admin.study_materials.pdfs.store') }}", formData)
                            .then(response => {
